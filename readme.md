...
## Current Status and problems for different platforms
### Windows 
#### Native Child Window Mode :heavy_check_mark: 
#### Offscreen Rendering Mode :heavy_check_mark: 
...
#### Native Child Window Mode :x: 
    1. embedded window size problem
#### Offscreen Rendering Mode :heavy_check_mark: 


## About OSR mode

QCefView supports CEF offscreen rendering mode, to enable OSR mode just add switch `USE_OSR=ON` during the CMake configuring stage (remember to clear the cache generated by previous configuration).
